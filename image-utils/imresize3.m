...
if isPureNearestNeighborComputation(weights{1}) && ...
        ...
        isPureNearestNeighborComputation(weights{3})
    ...
    for k = 1:numel(order)
        ...
        B = resizeAlongDimUsingNearestNeighbor(B, dim, indices{dim});
    end
...
%------------------------------------------------------------------
%
% here is where the image is loaded
%
%------------------------------------------------------------------
...
if isnumeric(arg) && isscalar(arg)
    ...
    if ~isempty(strfind(version, '2014'))
        validateattributes(arg, {'numeric'}, {'nonzero', 'real'}, mfilename, 'SCALE', position);
    else
        iptcheckinput(arg, {'numeric'}, {'nonzero', 'real'}, mfilename, 'SCALE', position);
    end
...
%------------------------------------------------------------------
%
% output_size must have three elements
%
%------------------------------------------------------------------
...
elseif isnumeric(arg) && isvector(arg) && (numel(arg) == 3)
    ...
    if ~isempty(strfind(version, '2014'))
        validateattributes(arg, {'numeric'}, {'vector', 'real', 'positive'}, mfilename, '[MROWS NCOLS]', position);
    else
        iptcheckinput(arg, {'numeric'}, {'vector', 'real', 'positive'}, mfilename, '[MROWS NCOLS]', position);
    end
...
if ~isempty(scale_in) && isscalar(scale_in)
    scale = [scale_in scale_in scale_in];
else
    scale = scale_in;
end
%---------------------------------------------------------------------
...
A_3d_size = [size(params.A, 1) size(params.A, 2) size(params.A, 3)];
output_size = ceil(params.scale .* A_3d_size);
%---------------------------------------------------------------------
...
if ~isempty(params.size_dim)
    ...
    A_2d_size = [size(params.A, 1) size(params.A, 2) size(params.A, 3)];
