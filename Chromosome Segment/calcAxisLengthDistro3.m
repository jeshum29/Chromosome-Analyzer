function gauss_axis_len = calcAxisLengthDistros(data)
...
lens = lens ./ repmat(sum(lens,2), 1, 6);

% one volume is now redundant
lens = lens(:,1:5);

C = calc_cov_matrix(lens);
len_mean = mean(lens,1);

gauss_axis_len.M = len_mean;
gauss_axis_len.C = inv(C);
